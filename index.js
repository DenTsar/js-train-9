...
function findElementGreaterThan(arr, num) {
  ...
  return !Array.isArray(arr) ? undefined : arr.find((element) => element > num);
}
...
function findIndexByProperty(arr, name) {
  ...
  return !Array.isArray(arr)
    ? -1
    : arr.findIndex((element) => element.name === name);
}
...
function findLastEvenNumber(arr) {
  ...
  return !Array.isArray(arr)
    ...
    : arr.findLast((element) => !(element % 2)) >= 0
    ? arr.findLast((element) => !(element % 2))
...
function getPositiveArrayValues(arr) {
  ...
  let positiveArrayValues = arrV.filter((el) => el > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  arr.splice(startIdx);
  let doubledArray = arr.map((el) => el * 2);
  return doubledArray;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  let fArr = arr.flat();
  fArr = fArr.map((element) => element ** 2);
  fArr = fArr.filter((element) => element > threshold);
  return fArr;
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  const nArr = arr.slice(-n);
  const sum = nArr.reduceRight((sum, elem) => sum + elem);
  nArr.forEach((elem) => console.log(elem));
  return sum;
}
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) && n.type === Number)
  ...
  const expandedArray = arr.flatMap((elem) => FnN(elem));
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((acc, elem) =>
    elem.length > acc.length ? (acc = elem) : 0
  );
  return longestWord;
}
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter(
    (elem, index) => index != arr.indexOf(elem)
  );
...
function capitalizeWords(arr) {
  ...
  if (!Array.isArray(arr)) return [];
  const Fn = (elem) => {
    return elem.slice(0, 1).toUpperCase().concat(elem.slice(1, elem.length));
  };
  const capitalizedArray = arr.map((elem) => Fn(elem));
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((total, val) => (total = total + val.price), 0);
  return totalPrice;
}
